...
if (!canvas) {
    ...
    // Only add scroll handling if sections exist (i.e., on index.html)
    if (document.querySelectorAll('section').length > 0) {
        window.addEventListener('wheel', (event) => {
            ...
            if (currentSection) {
                ...
                if (currentSection.id === 'contact' && event.deltaY > 0) {
                    return
                }
            }
            event.preventDefault();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Only fetch GitHub repos if repo-container exists (i.e., on index.html)
    if (repoContainer) {
